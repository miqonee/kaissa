...
const isPlaying = computed(() => isParticipant.value && state.value?.status === 'active');
const isViewing = computed(() => !isPlaying.value);
...
function playerOfBoard(b: number, color: 'w' | 'b'): GameParticipantInfo | undefined {
  return state.value?.participants.find((x) => x.boardIndex === b && x.color === color);
}
...
onMounted(() => {
  socket.on('game:state', (st) => {
    ...
    if (st.chat && Array.isArray(st.chat) && chat.value.length === 0) {
      chat.value = [...st.chat];
      scrollChat();
    }
  ...
  socket.on('game:chat', (msg) => {
    ...
    chat.value.push(msg);
    if (isViewing.value && !isChatOpen.value && msg.userId !== myId.value) {
  ...
  api.get<{ state: GameState }>(`/api/games/${gameId}/state`).then((res) => {
    if (res.state) {
      ...
      if (res.state.chat && Array.isArray(res.state.chat) && chat.value.length === 0) {
        chat.value = [...res.state.chat];
        scrollChat();
      }
  ...
  <div v-if="state" class="game-page" :class="state.mode">
    ...
    <div class="game-layout">
      ...
      <section class="boards-container" aria-label="Шахматные доски" :class="{ 'two-boards': state.mode === 'bughouse' }">
          ...
          <div class="team-players-bar top">
            ...
            <div class="team-slot left">
              ...
              >
                ...
                <BotHoverCard
                  v-if="teamModeSides.top.players[0].isBot"
                  :username="teamModeSides.top.players[0].username"
                  :rating="teamModeSides.top.players[0].ratingBefore"
                >
                  <span class="bot-badge tiny">
            ...
            <div class="team-slot right">
              ...
              >
                ...
                <BotHoverCard
                  v-if="teamModeSides.top.players[1].isBot"
                  :username="teamModeSides.top.players[1].username"
                  :rating="teamModeSides.top.players[1].ratingBefore"
                >
                  <span class="bot-badge tiny">
          ...
          <div class="team-players-bar bottom">
            ...
            <div class="team-slot left">
              ...
              >
                ...
                <BotHoverCard
                  v-if="teamModeSides.bottom.players[0].isBot"
                  :username="teamModeSides.bottom.players[0].username"
                  :rating="teamModeSides.bottom.players[0].ratingBefore"
                >
                  <span class="bot-badge tiny">
            ...
            <div class="team-slot right">
              ...
              >
                ...
                <BotHoverCard
                  v-if="teamModeSides.bottom.players[1].isBot"
                  :username="teamModeSides.bottom.players[1].username"
                  :rating="teamModeSides.bottom.players[1].ratingBefore"
                >
                  <span class="bot-badge tiny">
        ...
        >
          <div class="board-top-info">
            ...
            <span class="bughouse-board-players dim small">
              {{ playerOfBoard(bc.index, 'w')?.username ?? 'Белые' }} (Б) vs {{ playerOfBoard(bc.index, 'b')?.username ?? 'Чёрные' }} (Ч)
            </span>
          </div>
      ...
      <aside class="game-sidebar">
            ...
            <div v-if="state.mode === 'team'" class="queue-explain">
              <div class="queue-list">
                ...
                >
                  ...
                  <BotHoverCard v-if="p.isBot" :username="p.username" :rating="p.ratingBefore">
                    <span class="queue-name">{{ displayName(p) }}</span>
                  </BotHoverCard>
                  <span v-else class="queue-name">{{ displayName(p) }}</span>
        ...
        <!-- Вариант А: Игрок играет активный матч -> открытый чат на всю высоту (без статистики) -->
        <div v-if="isPlaying" class="panel chat-panel">
          <div class="panel-head">
        ...
        <!-- Вариант Б: Просмотр матча (зритель или оконченная игра) -> Дебют и анализ + шторка чата -->
        <template v-else>
          ...
          <!-- Выдвижная шторка чата для режима просмотра -->
...
/* ================= ПАНЕЛЬ ДЕБЮТА (В РЕЖИМЕ ПРОСМОТРА) ================= */
.opening-panel {
  flex: 1;
  display: flex;
  flex-direction: column;
...
/* ================= ВЫДВИЖНОЙ ЧАТ ДЛЯ РЕЖИМА ПРОСМОТРА ================= */
.chat-drawer-wrapper {
  height: 40px;
  min-height: 40px;
  flex: none;
